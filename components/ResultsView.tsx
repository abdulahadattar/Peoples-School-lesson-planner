  ...
  if (lessonPlans.length > 0) {
    ...
    return (
      <div className="h-full flex flex-col bg-brand-bg">
        <div className="flex-shrink-0 px-4 py-3 bg-brand-surface/80 dark:bg-brand-surface/60 backdrop-blur-xl border-b border-brand-border/60 flex items-center justify-between sticky top-0 z-10">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-6">
          <div className="max-w-3xl mx-auto space-y-5">
            <div className="bg-white/95 dark:bg-slate-800/95 backdrop-blur-xl rounded-2xl shadow-card border border-brand-border/80 p-5">
              <h1 className="text-xl font-bold text-brand-text-light mb-2 tracking-tight">{selectedPlan.title}</h1>
              <div className="flex flex-wrap gap-2 text-xs text-brand-text-medium">
                <span className="px-2.5 py-1 bg-brand-bg/60 rounded-lg border border-brand-border/60 font-medium">{selectedPlan.gradeLevel}</span>
                <span className="px-2.5 py-1 bg-brand-bg/60 rounded-lg border border-brand-border/60 font-medium">{selectedPlan.subject}</span>
              </div>
            </div>

            <div className="bg-white/95 dark:bg-slate-800/95 backdrop-blur-xl rounded-2xl shadow-card border border-brand-border/80 p-5">
              <h2 className="text-sm font-bold text-brand-primary uppercase tracking-widest mb-3">Objective</h2>
              ...
              <div className="bg-white/95 dark:bg-slate-800/95 backdrop-blur-xl rounded-2xl shadow-card border border-brand-border/80 p-5">
                ...
                <div className="space-y-4">
                  {selectedPlan.activities.map((activity, i) => (
                    <div key={i} className="border-l-2 border-brand-primary/60 pl-4">
                      <div className="flex items-center gap-2 mb-2">
                        <h3 className="font-bold text-brand-text-light text-sm">{activity.name}</h3>
                        <span className="text-[11px] text-brand-text-medium bg-brand-bg/60 px-2 py-0.5 rounded-md font-mono">{activity.duration} mins</span>
                      </div>
                      <p className="text-sm text-brand-text-medium leading-relaxed mb-2">{activity.description}</p>
                      {activity.teacherActions && (
                        <div className="mb-1.5">
                          <span className="text-[11px] font-bold text-brand-primary uppercase tracking-wider">Teacher Actions:</span>
                          <p className="text-sm text-brand-text-light leading-relaxed">{activity.teacherActions}</p>
                        </div>
                      )}
                      {activity.studentResponses && (
                        <div>
                          <span className="text-[11px] font-bold text-brand-primary uppercase tracking-wider">Student Responses:</span>
            ...
            <div className="bg-white/95 dark:bg-slate-800/95 backdrop-blur-xl rounded-2xl shadow-card border border-brand-border/80 p-5">
              <h2 className="text-sm font-bold text-brand-primary uppercase tracking-widest mb-3">Resources</h2>
      ...
      <div className="h-full flex flex-col bg-brand-bg">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-6">
          <div className="max-w-3xl mx-auto">
            <div className="bg-white/95 dark:bg-slate-800/95 backdrop-blur-xl rounded-2xl shadow-card border border-brand-border/80 p-5 mb-6 text-center">
              <h1 className="text-lg font-bold text-brand-text-light mb-1 tracking-tight">{schoolName}</h1>
              <h2 className="text-xl font-bold text-brand-text-light mb-3 tracking-tight">{paper.title}</h2>
              <div className="flex flex-wrap justify-center gap-2.5 text-xs text-brand-text-medium">
                <span className="px-2.5 py-1 bg-brand-bg/60 rounded-lg border border-brand-border/60 font-medium">Subject: {paper.subject}</span>
                <span className="px-2.5 py-1 bg-brand-bg/60 rounded-lg border border-brand-border/60 font-medium">Class: {paper.gradeLevel}</span>
                <span className="px-2.5 py-1 bg-brand-bg/60 rounded-lg border border-brand-border/60 font-medium">Total Marks: {paper.totalMarks}</span>
                <span className="px-2.5 py-1 bg-brand-bg/60 rounded-lg border border-brand-border/60 font-medium">Duration: {paper.durationMinutes} mins</span>
              </div>
            </div>

            {paper.sections.map((section, sIdx) => (
              <div key={sIdx} className="bg-white/95 dark:bg-slate-800/95 backdrop-blur-xl rounded-2xl shadow-card border border-brand-border/80 p-5 mb-4">
                <h3 className="text-lg font-bold text-brand-text-light mb-1.5 tracking-tight">{section.title}</h3>
